...
function undock_by_bay_index(carrier_vehicle, bay_index)
    if carrier_vehicle:get_team() ~= g_spectator.current_team then
        return
    end
...
function update(screen_w, screen_h, ticks)
    ...
    if g_spectator.loaded == 0 then
        g_spectator.loaded = 1
        g_spectator.current_team = update_get_screen_team_id()
        print(string.format("current team is %d", g_spectator.current_team))
        if is_spectator() then
            print("we are the spectator team")
        end
    end
                ...
                for i = 0, vehicle_count - 1, 1 do 
                    ...
                    if vehicle:get() then
                        ...
                        if vehicle_definition_index ~= e_game_object_type.chassis_spaceship and vehicle_definition_index ~= e_game_object_type.drydock then
                            ...
                            if is_spectator() or vehicle_team == update_get_screen_team_id() then
